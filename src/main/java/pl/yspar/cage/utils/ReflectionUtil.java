...
public class ReflectionUtil {
    ...
    public static int getVersion(final Player p) {
        ...
        try {
            final Object handle = c.getMethod("getHandle", new Class[0]).invoke(cp);
            final Object playerConnection = handle.getClass().getField("playerConnection").get(handle);
            final Object networkManager = playerConnection.getClass().getField("networkManager").get(playerConnection);
            final Method m = networkManager.getClass().getMethod("getVersion", new Class[0]);
            return (int) m.invoke(networkManager, new Object[0]);
        } catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchFieldException |
                 SecurityException | NoSuchMethodException ex5) {
            ex5.printStackTrace();
            return 0;
        }
    }
    ...
    public static void sendPacket(final Object packet, final Player p) {
        try {
            ...
            sendPacket.invoke(playerConnection, packet);
                 ...
                 InvocationTargetException | NoSuchFieldException ex5) {
            ex5.printStackTrace();
        }
    }
